...
// @ts-ignore
import nodeFetch from '@khulnasoft/node-fetch'
...
export async function runRelay(
  ...
  //create network
  log('add network')
  const network = await new Network({ name: 'khulnasoft_network_' + id }).start()

  // create relay container
  log(`create relay ${slug + '-' + id}`)
  const relay = await new GenericContainer('khulnasoft/deno-relay:v1.5.0')
    .withName(slug + '-' + id)
